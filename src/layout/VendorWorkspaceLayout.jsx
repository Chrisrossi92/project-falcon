...
const navItems = Object.freeze([
  ...
  { label: "Current Assignments", path: "/vendor-workspace/assigned-orders", enabled: true },
  { label: "Historical Assignments", path: "/vendor-workspace/historical-assignments", enabled: true },
  { label: "Documents", path: "/vendor-workspace/documents", enabled: true },
  { label: "Credentials", path: "/vendor-workspace/credentials", enabled: true },
  { label: "Coverage/Profile", path: "/vendor-workspace/profile", enabled: true },
]);
...
export default function VendorWorkspaceLayout() {
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-x-hidden bg-slate-950 text-slate-950">
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-[17rem] flex-col border-r border-slate-800 bg-[radial-gradient(circle_at_18%_100%,rgba(37,99,235,0.22),transparent_42%),linear-gradient(180deg,#020617_0%,#07111f_38%,#111827_100%)] px-3 py-3 shadow-[18px_0_48px_rgba(2,6,23,0.36)] md:flex">
        <NavLink to="/vendor-workspace/dashboard" className="block w-64 rounded-lg opacity-95 transition hover:opacity-100">
          <img
            src={falconWordmark}
            alt="Falcon"
        ...
        <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-900/80 px-3 py-3">
          ...
          <div className="mt-1 text-sm font-semibold text-slate-100">Vendor Workspace</div>
          <div className="mt-2 inline-flex w-fit rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-blue-800">
            Vendor
          </div>
        </div>
        <nav aria-label="Vendor workspace sections" className="mt-5 flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto pb-3">
          {navItems.map((item) => (
            <NavLink
              key={item.label}
              to={item.path}
      ...
      <header className="sticky top-0 z-40 border-b border-slate-800 bg-slate-950/95 shadow-[0_10px_28px_rgba(2,6,23,0.18)] backdrop-blur-xl md:border-b-0 md:bg-transparent md:pl-[17rem] md:shadow-none">
        <div className="mx-auto flex min-h-14 max-w-[1500px] items-center justify-between gap-4 px-3 py-1.5 sm:px-4 md:min-h-12 md:py-2">
          <NavLink to="/vendor-workspace/dashboard" className="block w-52 rounded-lg opacity-95 transition hover:opacity-100 md:hidden">
            <img
              src={falconWordmark}
              alt="Falcon"
              className="-my-3 h-14 w-full translate-y-2 object-cover object-center"
          ...
          <div className="hidden min-w-0 md:block">
            ...
            <div className="text-sm font-semibold text-slate-100">Assignments, coverage, and credentials</div>
          </div>
        ...
        <nav aria-label="Vendor workspace mobile sections" className="flex gap-2 overflow-x-auto border-t border-slate-800 px-3 py-2 md:hidden">
          {navItems.map((item) => (
            <NavLink
              key={item.label}
              to={item.path}
