...
const migrationPath = resolve(
  repoRoot,
  'supabase/migrations/20260605100000_amc_vendor_workspace_assignment_document_access_rpc.sql',
);
...
describe('Vendor Workspace assignment document access authorization migration', () => {
  ...
  it('resolves only opaque assignment work and document keys in the current vendor company scope', () => {
    [
      "v_assignment_work_key !~ '^[0-9a-f]{64}$'",
      ...
      'ar.assignment_work_key = v_assignment_work_key',
  ...
  it('guards assigned vendor scope, active AMC vendor relationship/profile, AMC orders, and vendor-visible documents', () => {
    [
  ...
  it('keeps caller-facing authorization metadata vendor-safe', () => {
    ...
    const callerFunction = migrationSql.slice(
      migrationSql.indexOf('create or replace function public.rpc_vendor_workspace_authorize_assignment_document_access'),
      migrationSql.indexOf('create or replace function public.rpc_vendor_workspace_assignment_document_storage_lookup'),
    );
...
describe('Vendor Workspace assignment document download Edge function', () => {
  it('uses assignment caller authorization before service-role storage lookup and signed URL creation', () => {
    expect(edgeSource).toContain('assignment_work_key');
    expect(edgeSource).toContain('rpc_vendor_workspace_authorize_assignment_document_access');
    expect(edgeSource).toContain('rpc_vendor_workspace_assignment_document_storage_lookup');
    expect(edgeSource.indexOf('rpc_vendor_workspace_authorize_assignment_document_access')).toBeLessThan(
      edgeSource.indexOf('rpc_vendor_workspace_assignment_document_storage_lookup'),
    );
    expect(edgeSource).toContain('createSignedUrl');
    expect(edgeSource).toContain('SIGNED_URL_TTL_SECONDS');
  });

  it('accepts exactly one opaque work key kind plus an opaque document key and returns signed URLs without exposing storage fields', () => {
    expect(edgeSource).toContain('isOpaqueKey(body.work_key)');
    expect(edgeSource).toContain('isOpaqueKey(body.assignment_work_key)');
    expect(edgeSource).toContain('(hasWorkKey && hasAssignmentWorkKey)');
