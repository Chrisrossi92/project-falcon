...
export default function ClientDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Load AMC name if tied
        if (c?.amc_id) {
          try {
            // Prefer service if it includes relations; otherwise do a tiny direct select
  ...
  const totals = useMemo(() => {
    ...
    for (const o of orders) {
      ...
      const d = o?.created_at || o?.order_date || o?.assigned_at || null;
  ...
  async function handleUpdate(payload) {
    try {
      ...
      setClient((c) => ({ ...(c || {}), ...(updated || payload) }));
      // If we changed category → maybe clear or set AMC
      if ((payload.category || "").toLowerCase() === "amc") {
        setAmc(null);
      } else if (payload.amc_id) {
        // reload AMC name
        const { data } = await supabase.from("clients").select("id,name,category").eq("id", payload.amc_id).single();
        setAmc(data || null);
      }
    ...
    <div className="p-4 md:p-6">
      ...
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-1 text-xs text-gray-500">
            {client.contact_name ? <span>{client.contact_name}</span> : <span>—</span>}
            {client.contact_email ? (
              <>
                <span className="mx-1">•</span>
                <a className="underline" href={`mailto:${client.contact_email}`}>
                  {client.contact_email}
                </a>
              </>
            ) : null}
      ...
      <MergeClientsDialog
         ...
         onClose={(res) => {
           setMergeOpen(false);
           if (res?.mergedIntoId) {
             // navigate to the target record we kept
             navigate(`/clients/${res.mergedIntoId}`);
           }
         }}
       />
          ...
          <div className="rounded-xl border bg-white p-4">
            ...
            <dl className="mt-3 space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd className="text-gray-800">{client.contact_name || client.primary_contact || "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-500">Email</dt>
                <dd className="text-gray-800">{client.contact_email || client.email || "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-500">Phone</dt>
                <dd className="text-gray-800">{client.phone || "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-500">Status</dt>
                <dd className="text-gray-800">{client.status || "—"}</dd>
              </div>
              {client.notes ? (
                <div>
                    ...
                    {orders.map((o) => (
                      <tr key={o.id} className="border-t">
                        <td className="px-2 py-2">
                          <Link className="underline" to={`/orders/${o.id}`}>{o.order_no || o.id}</Link>
                        </td>
                        <td className="px-2 py-2">
                          <div className="truncate max-w-[280px]">
                            {o.address_line1 || o.address || "—"}
                            {o.city ? `, ${o.city}` : ""}{o.state ? `, ${o.state}` : ""}{o.zip ? ` ${o.zip}` : ""}
                          </div>
                        </td>
                        ...
                        <td className="px-2 py-2">{fmtDate(o.created_at)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
