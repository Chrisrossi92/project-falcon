...
export default function OwnerAssignmentManagement() {
  ...
  return (
    <section
      aria-label="Sent assignment packets"
    ...
    >
      <div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-50/70 px-4 py-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">Sent Assignments</div>
          <h2 className="mt-1 text-base font-semibold text-slate-950">Assignment packets offered by your company</h2>
          <p className="mt-1 max-w-2xl text-xs leading-5 text-slate-500">
            Owner-side packet oversight for work sent to relationship companies.
          </p>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          <select
            aria-label="Sent assignments status"
            value={status}
      ...
      {loading && <div className="p-4 text-sm text-slate-500">Loading sent assignment packets...</div>}
      {!loading && error && (
        <div className="p-4">
      ...
      {!loading && !error && visibleItems.length === 0 && (
        <div className="p-4">
          <EmptyState title="No sent assignments" message="No assignment packets have been offered by your company for this filter." />
        </div>
      )}
      {!loading && !error && visibleItems.length > 0 && (
        <div className="divide-y divide-slate-100">
          {visibleItems.map((item) => {
            ...
            const assignedCompany = item.assigned_company_name || "Assigned company";
            return (
              <Link
                key={item.id}
                to={`/assignments/${item.id}`}
                aria-label={`Open sent assignment packet for ${assignedCompany}`}
                className="group grid gap-3 px-4 py-3 transition hover:bg-slate-50 lg:grid-cols-[1fr_auto]"
              >
                <div className="min-w-0">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">Sent Packet</div>
                      <div className="mt-0.5 truncate text-base font-semibold text-slate-950">{assignedCompany}</div>
                    </div>
                  ...
                  <dl className="mt-3 grid gap-2 text-xs text-slate-500 sm:grid-cols-2">
                    ...
                    <div className="rounded-md bg-slate-50 px-2.5 py-2">
                      <dt className="font-semibold uppercase tracking-[0.12em] text-slate-400">Relationship</dt>
                      <dd className="mt-0.5 font-medium text-slate-700">{humanize(item.relationship_type)}</dd>
                    </div>
                  </dl>
                  {item.instructions && (
