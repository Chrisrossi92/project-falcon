...
## Phase 2G Local Storage Replay Compatibility Record

Phase 2G resolves the local Storage replay blocker by repairing the local Supabase environment
rather than adding Falcon-owned Storage catalog DDL.

The blocker occurred at:

- `20260518071000_order_documents_private_bucket_download_auth.sql`.

That migration references Supabase-managed Storage tables by:

- inserting/updating the private `order-documents` bucket in `storage.buckets`;
- relying on later document upload/finalize migrations that validate uploaded objects through
  `storage.objects`.

Nearby document migrations confirm the dependency:

- `20260518070000_order_documents_metadata_list_rpc.sql` creates Falcon-owned document metadata and
  does not require Storage catalog tables;
- `20260518072000_order_documents_upload_prepare_finalize.sql` validates finalized uploads against
  `storage.objects`;
- `20260518073000_order_documents_finalize_activity.sql` also relies on finalized
  `storage.objects` records.

The local Storage API container was healthy, but the local database had only the `storage` schema
and lacked `storage.buckets` and `storage.objects`. Phase 2G used the Supabase Storage image's own
tenant migrations to repair the local managed Storage schema, then granted the local migration role
access to the managed Storage schema for replay validation. Falcon application migrations were not
changed to create or redefine Supabase-managed Storage catalog tables.

After that local environment repair:

- `supabase migration up --local` replayed through document Storage migrations;
- replay applied `20260524090000_order_operational_inputs.sql`;
- `supabase migration list --local` showed `20260524090000` applied locally;
- direct local inspection confirmed `storage.buckets`, `storage.objects`, and the Phase 2E
  migration version exist.

During Phase 2G validation, `supabase db lint --local` found a direct Phase 2E issue in the new
operational input RPC activity logging: `_activity_actor()` exposes `full_name` and `email`, not
`actor_name` and `actor_email`. Because the Phase 2E migration is still uncommitted in this work
slice, `20260524090000_order_operational_inputs.sql` was corrected in place to use
`v_actor.full_name` and `v_actor.email`, matching existing activity-log RPC conventions. The
corrected migration was reapplied locally for validation.

Current validation state:

- the migration chain reaches and applies Phase 2E locally;
- Phase 2E-specific lint errors are cleared;
- `supabase db lint --local` still reports existing unrelated legacy/function issues, including
  older order numbering, notification, legacy activity, import, and client RPC findings;
- no frontend runtime, UI, lifecycle, automation, notification, route, dashboard, navigation,
  mobile, Client Portal, or signal-suppression behavior changed.

Phase 2E stop condition remains active. Do not proceed to UI, Attention Summary suppression,
mobile controls, notifications, automation, or dashboard behavior without a separate approved
runtime slice.

## Phase 2H Read-Only Surface Planning Record
...
Phase 2H defines the smallest safe frontend/read layer before create/clear UI:

- display active/fresh operational inputs only by default;
- use small read-only chips or rows on Order Detail or the order drawer;
- show labels for `inspection_scheduled`, `report_on_track`, and `waiting_on_client`;
- include actor, created timestamp, freshness, and optional safe note metadata only where useful;
- prefer existing RLS-backed reads if safe, or a separate read-only RPC/view if project
  conventions require it;
- avoid direct writes, create/clear controls, lifecycle/status mutation, Smart Action changes,
  signal suppression, automation, notifications, dashboard integration, mobile-specific behavior,
  and Client Portal exposure.

The recommended next implementation slice is:

- **Operational Execution Phase 2I: Read-Only Operational Input Display**.

Phase 2I should pause after proving read-only display behavior. Create/clear UI and Attention
Summary suppression must remain separate approved slices.
