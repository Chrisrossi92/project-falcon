...
| Workflow mutation | `rpc_transition_order_status(...)` through canonical `ordersService` helpers; order audit trigger owns `status_changed` activity | Smart Actions, drawer contextual duplicates, reviewer shortcut transitional duplicate | Generic status helpers, bulk status helpers, arbitrary status adapters, freeform status panel, direct `orders.status` writes | Direct status writes blocked; generic helper reachability blocked; direct transition RPC calls confined to `ordersService.js` | Frontend workflow notifications; frontend review/revision notes; reviewer shortcut duplication; `request_final_approval` notification semantics | Backend-owned workflow notifications; backend-owned atomic review/revision notes; reviewer shortcut consolidation |
