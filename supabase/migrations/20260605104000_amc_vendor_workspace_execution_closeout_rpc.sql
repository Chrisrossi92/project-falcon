...
begin
  ...
  vendor_assignment_rows as (
    ...
    where oca.assigned_company_id = v_vendor_company_id
      ...
      and oca.status in ('offered', 'accepted', 'in_progress', 'submitted', 'revision_requested')
  ...
  count_values as (
    select
      ...
      (select count(*)::integer from vendor_assignment_rows where status in ('accepted', 'in_progress', 'revision_requested')) as active_assigned_orders,
      ...
      (
        ...
        + (select count(*)::integer from vendor_assignment_rows where status = 'revision_requested')
        + (select count(*)::integer
  ...
  vendor_assignment_rows as (
    select
      case
        ...
        when oca.status = 'revision_requested' then 'revision_request'
        else 'submitted'
      end as kind,
      case
        ...
        when oca.status = 'revision_requested' then 'Respond to revision request'
      ...
      case
        when oca.status = 'revision_requested' then 'due_soon'
        when coalesce(
          case
            when oca.status = 'offered' then oca.expires_at
            when oca.status in ('accepted', 'in_progress') then oca.due_at
      ...
      case
        ...
        when oca.status = 'revision_requested' then 35
    ...
    where oca.assigned_company_id = v_vendor_company_id
      ...
      and (
        ...
        or oca.status in ('in_progress', 'submitted', 'revision_requested')
      )
      and coalesce(o.company_id, public.default_company_id()) = oca.owner_company_id
      and coalesce(o.operations_scope, 'internal_operations') = 'amc_operations'
  ),
...
comment on function public.rpc_vendor_workspace_dashboard_summary() is
  'AMC-10I Vendor Assigned Order Execution closeout dashboard summary. Keeps authenticated Vendor Workspace scoping from AMC-9B.2 and additionally counts revision_requested vendor assignments as active assigned orders and needs-attention work, with a vendor-native revision action label. Does not mutate bids, assignments, orders, public token flows, routes, navigation, shared order APIs, or expose raw ids/storage paths/internal notes/fees/margins/candidate data.';
