...
comment on function public.rpc_vendor_directory_list(text, text) is
  'AMC-2T read-only Vendor Directory list RPC. Uses current_company_id(), active current-company membership, and vendors.read authorization. Operations mode, product mode, and company_type are not authorization. Does not grant order, client, assignment, notification, or workflow visibility.';

comment on function public.rpc_vendor_profile_detail(uuid) is
  'AMC-2T read-only Vendor Directory profile detail RPC. Scoped to current_company_id() and vendors.read authorization. Staged profiles without relationship_id can be read by authorized current-company users. Does not return assignment packets or order data.';

comment on function public.rpc_vendor_profile_contacts(uuid) is
  'AMC-2T read-only Vendor Directory contacts RPC. Scoped through the parent vendor profile and vendors.read authorization. Contact user_id linkage does not grant vendor portal access or assignment notification routing.';

comment on function public.rpc_vendor_profile_service_areas(uuid) is
  'AMC-2T read-only Vendor Directory service areas RPC. Scoped through the parent vendor profile and vendors.read authorization. Service areas do not create assignments; vendor assignment remains in order_company_assignments.';
