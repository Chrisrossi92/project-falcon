    ...
    if v_send_now then
      ...
      if v_sent_to_email is null
        ...
        v_email_message := concat_ws(
          ...
          '',
          'Requested Timeline',
          'Client due date: ' || coalesce(to_char(coalesce(v_bid_request.client_due_at, v_order.client_due_at, v_order.final_due_at, v_order.due_date::timestamptz) at time zone 'UTC', 'FMMonth FMDD, YYYY'), 'Not set'),
          'Vendor due date: ' || coalesce(to_char(v_bid_request.desired_vendor_due_at at time zone 'UTC', 'FMMonth FMDD, YYYY'), 'Not set'),
          'Response deadline: ' || coalesce(to_char(v_bid_request.response_due_at at time zone 'UTC', 'FMMonth FMDD, YYYY "at" FMHH12:MI AM "UTC"'), 'Not set'),
          '',
          ...
          '',
          'Submit Bid',
          'Use the secure bid link in this email to submit your fee, turn time, proposed due date, and comments.'
        );
...
comment on function public.rpc_order_vendor_bid_request_create(uuid, jsonb) is
  'AMC-11C bid request email experience alignment. Requires amc_operations order scope, bid_requests.create, vendors.read, current-company order read authority, active amc_vendor relationships, eligible vendor profiles, and no duplicate open recipient. Creates bid request/recipient rows, queues vendor-safe bid invitation emails with tokenized public links, coordinator message, structured VENDOR_BID_INVITATION payload, human-readable fallback content, conservative match reason, and no-documents placeholder when recipient email exists; preserves manual link fallback when email is missing; does not create assignments, mutate orders, expose fees/margins/internal notes, or send email directly.';
