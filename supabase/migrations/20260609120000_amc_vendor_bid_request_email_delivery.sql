    ...
    if v_send_now then
      ...
      if v_sent_to_email is null
        ...
        insert into public.email_queue (
          ...
          null,
          v_sent_to_email,
          concat('Bid request: ', coalesce(nullif(v_order.property_address, ''), nullif(v_order.address, ''), 'Appraisal assignment')),
...
comment on function public.rpc_order_vendor_bid_request_create(uuid, jsonb) is
  'AMC-11 bid request create RPC. Requires amc_operations order scope, bid_requests.create, vendors.read, current-company order read authority, active amc_vendor relationships, eligible vendor profiles, and no duplicate open recipient. Creates bid request/recipient rows, queues vendor-safe bid invitation emails with tokenized public links when recipient email exists, preserves manual link fallback when email is missing, and does not create assignments, mutate orders, expose fees/margins/internal notes, or send email directly.';
