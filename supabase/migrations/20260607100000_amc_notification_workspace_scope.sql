-- AMC-14B: notification secondary surfaces must respect the selected operations workspace.
...
comment on function public.notification_row_matches_operations_scope(public.notifications, text) is
  'AMC-14B helper for notification workspace isolation. Order notifications use orders.operations_scope; scoped payload notifications use payload scope fields; unscoped non-order system notifications remain global.';

comment on function public.rpc_get_notifications(integer, timestamptz, text) is
  'AMC-14B scoped notification list. Optional p_operations_scope prevents Internal/AMC notification bleed while preserving current user and order-read authorization.';

comment on function public.rpc_get_unread_count(text) is
  'AMC-14B scoped unread notification count for the selected operations workspace.';

comment on function public.rpc_mark_all_notifications_read(text) is
  'AMC-14B scoped bulk read action. Only notifications visible in the selected operations workspace are marked read.';

comment on function public.rpc_dismiss_seen_notifications(text) is
  'AMC-14B scoped dismiss-seen action. Only seen notifications visible in the selected operations workspace are dismissed.';
