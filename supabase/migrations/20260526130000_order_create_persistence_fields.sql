...
comment on function public.rpc_create_order(jsonb) is
  'Guarded order creation RPC. Generates order number server-side, snapshots selected client relationship contact fields, and persists order intake/report/due date fields from payload.';
