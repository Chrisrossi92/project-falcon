...
comment on function public.rpc_create_order(jsonb, text) is
  'Guarded order creation RPC. Generates order number server-side, snapshots selected client relationship contact fields, explicitly writes operations_scope, and restricts create status to safe initial status.';
